...
import { getOutputs as getNftOutputs } from "../backend-libs/screenshot-nft/lib";
...
async function getImages(options:TapesRequest) {
  const res:DecodedIndexerOutput = await getOutputs(
    {
        tags: ["screenshot"],
        type: 'report',
  ...
  return verificationINputs.map((val: Uint8Array) => byteToBase64(val));
}
...
export default function Nfts() {
  ...
  async function nextPage() {
    ...
    promises.push(getImages(tapesRequestOptions));
    ...
    .then((values) => {
      ...
      // score
      let tapesScores:VerificationOutput[] = values[1];
      let tapeId;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {
            verificationInputs?.map((verificationInput, index) => {
              ...
              return (
                <Link key={index} href={`/nfts/${tapeId}`} className="relative">
                  <div 
